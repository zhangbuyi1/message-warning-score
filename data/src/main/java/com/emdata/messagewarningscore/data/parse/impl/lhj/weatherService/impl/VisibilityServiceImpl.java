...
@Slf4j
@Service
public class VisibilityServiceImpl extends AlertCommonAdaptee implements WeatherService {
...
//        alertContent.setReadableTextContent(textContent);
    ...
    @Override
    public void setMainWeatherValue(AlertContentResolveDO airportAlertContent, String mainStr, String unit){
        if (mainStr.contains(unit)) {
            String mainValue = mainStr.replace(unit, "");
            if (mainValue.contains("-")) {
                String[] split1 = mainValue.split("-");
//                airportAlertContent.setWeatherMinNum(deleteSymbolForVisibility(split1[0]));
//                airportAlertContent.setWeatherMaxNum(deleteSymbolForVisibility(split1[1]));
            }else{
                if (mainValue.contains("以上")) {
//                    airportAlertContent.setWeatherMinNum(deleteSymbolForVisibility(mainValue));
                }else {
//                    airportAlertContent.setWeatherMaxNum(deleteSymbolForVisibility(mainValue));
                }
            }
        }
    }
    ...
    @Override
    public void setAdditionalWeatherValue(AlertContentResolveDO airportAlertContent, String additionalStr, String unit){
...
//                airportAlertContent.setAdditionalMinNum(deleteSymbolForVisibility(split1[0]));
//                airportAlertContent.setAdditionalMaxNum(deleteSymbolForVisibility(split1[1]));
            }else{
                if (additionalValue.contains("以上")) {
//                    airportAlertContent.setAdditionalMinNum(deleteSymbolForVisibility(additionalValue));
                }else {
//                    airportAlertContent.setAdditionalMaxNum(deleteSymbolForVisibility(additionalValue));
                }
            }
        }
    }
    ...
    // 删除特别的符号（只针对能见度）
    public static String deleteSymbolForVisibility(String inStr){
        String outStr = inStr;
